...
- **Step 4 — priced path:** on Accept → rolling per-project CO invoice (create on first accepted CO, append lines, `source_type='change_order'`; QB push mirrors the contract flow) + receivable row in the payment section + financials re-tally. `co_invoice_id` (067) links a CO to its rolling invoice.
