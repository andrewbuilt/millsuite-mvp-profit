...
**The ONLY active work is the migration test projects + the QB-mapping work.** Status: **6a-2 (Subproject parity + QB mapping) is BUILT + verified in the app** (064 run on prod, items synced, DPR maps); **next build is 6a-3 (QB-activity prefills: install block + payment terms + autofill), scoped + decisions locked** (see "Now"). Then the "skip finished projects" change (Arnold/Pendry drop) and the 6b re-run/check loop. Live active test job is **DPR**. Until Andrew signs off:
...
**6a-3. QB-activity prefills: install block + payment terms + description autofill — scoped 2026-07-18 (planning pass w/ Andrew; decisions locked). Build next.**

Context: Built models install as ONE project-level block (QB activity + men×days×hrs×rate×complexity → its own invoice line) and payment terms as a QB-activity "prefill" subproject (select the activity → description fills from the QB item's text → appears as a $0 inline line). MillSuite instead folds install into each subproject's price (no install line) and has no activity-driven prefill. The migration carried Built's "Installation" + "Millwork Payment Terms" subs across as plain subprojects (the dashed-purple INSTALL badge is just MillSuite's `isInstallSub` name heuristic — wired to neither pricing system). **Verified: the 29 synced `qbo_items_cache` rows already carry the prefill text** (Millwork Payment Terms = the deposits/draws boilerplate, Delivery/Install/PM = install terms, Millwork/-Cabinets = material template) — so the whole thing keys off `qbo_items_cache.description`.

**Decisions locked (Andrew):** (1) install reaches the invoice via **BOTH** a project-level block AND a per-sub "includes install" checkbox (guard against double-counting — a sub with the checkbox on isn't also swept into the block); (2) the migrated "Installation" sub **converts to the new project install block** (migration maps Built install subs → the block going forward); (3) "Millwork Payment Terms" behaves as a **$0 inline invoice line** whose activity type autofills the terms text, shown alongside the existing milestone schedule.

**Build (3 pieces, commit each):**
1. **Activity-type → description autofill** (foundation, reused by all): in the subproject `ScopeEditor`, selecting a QB activity type offers to fill the Description blocks from that item's cached `description` (split into blocks) + a **"Restore default"** action to re-pull. Don't clobber non-empty edits silently (confirm/optional). Ports Built's SubprojectCard/InstallSubprojectCard autofill.
2. **Project-level install block** (`app/(app)/projects/[id]/page.tsx` + a new install card): an "Add install" button (mirrors Built's "Installation added") → a dedicated install card — QB install activity type (default 'Delivery, Install & Project Management - Commercial'), men/days/hrs-per-day/rate/complexity inputs, autofilled install description → **its own line** on the invoice/estimate + QB push (ItemRef from the install activity). Reuses `lib/install-prefill.ts` math (add an install-rate field; Built defaults 68/hr vs MillSuite's shop rate — confirm which). Convert the migrated Installation sub into this block.
3. **Per-sub "includes install" checkbox**: on the existing per-sub install prefill, a checkbox that promotes that sub's install into its own invoice line (instead of silently folding into the sub price). Off = current behavior (folded). Guard: a checked sub is not also counted by the project block.

**Invoice/estimate impact:** install + payment-terms now emit real QB lines (install → its ItemRef; payment terms → its ItemRef at $0). Watch the project rollup / `bid_total` so install isn't double-counted (folded-in vs its own line) and the $0 terms line doesn't skew totals. Verify a DPR-style push preview shows: millwork subs, one install line, a $0 payment-terms line.
