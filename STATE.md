...
- **Step 4b — QB push (NOT built; Andrew's live check gates it):** in QB mode, push/update the rolling CO invoice to QuickBooks mirroring the contract flow. Needs a decision: QB gets ONE rolling CO invoice that we **update** on each accept (push-invoice only *creates* today — needs QB update support), vs. a simpler "push the CO invoice to QB" button the operator hits when ready. Confirm with Andrew before writing — nothing hits QB until he signs off on 4a.

**Remaining (heavier — money/QB + a migration; build carefully, not rushed):**
- **Step 7 — documents section:** project-detail Documents (estimate / contract invoice / CO invoice / Drive links + "+ document link") — needs a small idempotent `project_documents` migration.
...
- **Latest DB migration is `066`** (`066_subproject_install_included.sql`, per-sub install opt-in — **run on prod 2026-07-18**; `065` install rate + `064` qbo_items_cache also run 2026-07-18). Run any new migration against prod Supabase before deploying.
