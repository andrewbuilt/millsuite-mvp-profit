...
#### Build (commit each)
1. ✅ **Mount + entry point** (`d0da11c`): "Change order" button in the subproject header (post-sold) + change-orders list on the project page (Option A additive total). ✅ **Two-mode modal to the approved prototype** (`specs/change-orders/co-modal-prototype.html`) — one modal, **Spec change | Custom** segmented toggle, shared title (auto) + Drawing-rev / No-charge + net footer + "Create as draft" (`a0032fb`; earlier iters `294cb8d`/`a8aaa07`/`ed8485d`). **Custom mode = fully built** (free desc, multiple material lines + "Vendor item" toggle, per-dept labor → shop rate, client price through margins editable, $0=doc-only; materials in snapshot notes). **Spec-change mode = interim** (split-spec picker + rate-book material type-ahead + unit-cost delta × qty-from-line). **Follow-up (next slice): real composer-slot integration for spec mode** — "What's changing?" derived from the product's slot defs (drawers included), Original→Proposed = per-slot rate-book options + "+ Add new" (shared composer add-material flow, writes the rate book), net = composer math, automatic labor delta. Reuse the old `lib/change-orders` composer machinery (`computeNetChange`/`draftCoFromApprovalCard`/`applyApprovedCo`). Retire the old per-line CO buttons + standalone `CreateCoModal` once spec mode is real + signed off.
