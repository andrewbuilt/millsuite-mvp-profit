...
**Step 3 — free path — BUILT 2026-07-21 (`ea36782`, tsc clean):** a $0 CO now applies on creation instead of waiting for Send/Accept. `finalizeFreeCo` in `lib/change-orders` (state→approved + `applyApprovedCo` spec/card propagation + bid recompute; **kept separate from `approveCo` so step 4's invoice logic never touches the free path**). `CreateChangeOrderModal`: `clientPrice===0` → `finalizeFreeCo` right after create; button reads "Create & apply" (vs "Create as draft"). Project-page CO row now surfaces the **drawing-revision-required** flag (was PDF-only) + an "Applied · no invoice" chip. NO invoice created (step 4 not built yet anyway). _Left for Andrew: live QA — create a $0 spec CO on a sold sub, confirm it applies immediately, the linked approval card reflects the new spec, drawing chip shows, no invoice appears. Note: custom-mode dept-hours→schedule propagation on accept is still unbuilt (Path B piece, applies to both free + priced) — not part of step 3._
