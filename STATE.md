...
**Progress (2026-07-18):** **chunks 1–2 built** (`92a1de5`) — `scripts/migrate-built/` scaffold (env → both Supabase clients, CLI `--dry-run/--entity/--project/--limit`, `id-map.ts` upsert-through-map, `migrate.ts` pipeline with chunk 3–5 stubs, `dump-schema.ts` schema-only exporter) + migration **`063_migration_id_map.sql`** (migration_id_map + `projects.built_archive`). MillSuite creds load from `.env.local`; Built creds via gitignored `scripts/migrate-built/.env`. Verified: tsc clean, `npx tsx` runs, preflight flags missing 063, scripts gate cleanly on absent creds. _Discovered a supabase-js gotcha (documented in `migrate.ts`): a head/count query does NOT error on a missing table, so preflight uses a real select._ **Blocked on Andrew for the next step:** (a) **run `063_migration_id_map.sql` on MillSuite prod**; (b) provide **Built OS Supabase URL + service key** (→ `scripts/migrate-built/.env`) so chunk 1's `dump-schema.ts` can actually run against live Built. Chunks 3–6 below are next.
...
- **Latest DB migration is `063`** (`063_migration_id_map.sql`, Built OS migration id map + `projects.built_archive` — **written 2026-07-18, NOT yet run on prod**; `062_pto.sql` PTO ran 2026-07-17). Run any new migration against prod Supabase before deploying.
