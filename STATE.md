...
**The ONLY active work is the migration test projects + the QB-mapping work.** Status: **6a-2 BUILT + verified** (064 run, items synced, DPR maps); **6a-3 (QB-activity prefills: install block + payment terms + autofill) BUILT** — pieces 1–2 pushed + verified, **piece 3 committed but holding push until migration `066` runs on prod**. Then the "skip finished projects" change (Arnold/Pendry drop) and the 6b re-run/check loop. Live active test job is **DPR**. Until Andrew signs off:
...
3. **Per-sub "includes install" checkbox** (`ee9664c`, **NOT pushed — needs `066`**): InstallPrefill card gains an "Includes install" checkbox — off = 0 install cost + 0 hours (inputs hidden), on = billed with the sub. **migration `066` `install_included`** (default false; backfills true where install already set). `computeInstallCost`/`Hours` return 0 when `included===false`; threaded through project-totals/rollup/handoff/schedule-seed; "Add install" + migration script set it true. No double-count: each sub owns its install once; the block is just a designated install sub.

_**Left for Andrew:** run **`066_subproject_install_included.sql`** on prod → then I push piece 3. Then test: "Add install" button, install Rate/hr, and the "Includes install" checkbox (off zeroes a sub's install)._
...
- **Latest DB migration is `066`** (`066_subproject_install_included.sql`, per-sub install opt-in — **NOT yet run on prod**; `065_subproject_install_rate.sql` install rate override ran 2026-07-18; `064_qbo_items_cache.sql` ran 2026-07-18). Run any new migration against prod Supabase before deploying.
