...
// ============================================================================
// BOM MODE — the purchasing parse (scoped item 2, 2026-09-12).
// ============================================================================
// ⛔ REUSES `uploadToParseBucket` AND THE SAME ENDPOINT ON PURPOSE. A second
// upload path would be a second place for the storage prefix, the bearer
// token and the daily cap to drift. The only difference on the wire is
// `mode: 'bom'`, which swaps the prompt pair server-side.
//
// ⚠️ Deliberately NOT routed through `parsePdfFile`. That function's job is to
// degrade gracefully into a regex fallback so the SALES flow always gets
// something to work with. There is no regex fallback for a bill of materials —
// a made-up count is worse than no count, because someone orders against it.
// This fails cleanly instead.

export interface BomParseResult {
  items: Array<{
    category: 'sheet_good' | 'hardware' | 'drawer' | 'other'
    name: string
    spec: string | null
    qty: number
    unit: string
  ...
  error: string | null
  /** True when the daily parse cap refused the call — a different message
   *  from a failure, because waiting fixes it. */
  rateLimited: boolean
}

export async function parsePdfForBom(file: File, orgId: string): Promise<BomParseResult> {
  const isPdf = /\.pdf$/i.test(file.name) || file.type === 'application/pdf'
  if (!isPdf) {
    return { items: [], error: 'That needs to be a PDF.', rateLimited: false }
  }

  // Same scanned-set detection as the estimate path — a drawing set exported
  // from a scanner has no text layer, and the server appends a vision-mode
  // addendum when told. Failure here is non-fatal.
  let isScanned = false
  try {
    const probe = await extractTextFromPdf(file)
    isScanned = probe.text.trim().length < TEXT_EMPTY_THRESHOLD
  ...
  const up = await uploadToParseBucket(file, orgId)
  if ('error' in up) {
    return { items: [], error: `Upload failed: ${up.error}`, rateLimited: false }
  }

  const sessionRes = await supabase.auth.getSession()
  const token = sessionRes.data.session?.access_token
  if (!token) {
    supabase.storage.from(PARSE_BUCKET).remove([up.path]).catch(() => {})
    return { items: [], error: 'Not signed in.', rateLimited: false }
  }
  ...
  try {
    resp = await fetch('/api/parse-drawings', {
      ...
      body: JSON.stringify({
        ...
        mode: 'bom',
        // ⛔ FALSE, so the server deletes the upload. It was true on the
        // reasoning that re-parsing is the workflow — but nothing records the
        // storage path for a BOM parse and there's no parse-from-stored-path
        // affordance, so "re-parse" means dropping the file again. Keeping it
        // just left a permanent unreferenced object in the bucket every run.
        keep_pdf: false,
      }),
    })
  } catch (err: any) {
    return { items: [], error: `Network error: ${err?.message || String(err)}`, rateLimited: false }
  }

  if (!resp.ok) {
    const body = await resp.text().catch(() => '')
    let message = `Parser error ${resp.status}`
    try {
    ...
    return { items: [], error: message, rateLimited: resp.status === 429 }
  }

  const json = await resp.json().catch(() => null)
  const items = Array.isArray(json?.bom) ? json.bom : []
  if (items.length === 0) {
    return {
      items: [],
      // ⚠️ An empty result is reported as such rather than as a success with
      // nothing in it — on a purchasing list those look identical and mean
      // very different things.
      error: 'The parser found nothing to buy in that set. Worth checking it’s the approved drawings.',
      rateLimited: false,
    }
  }
  return { items, error: null, rateLimited: false }
}
