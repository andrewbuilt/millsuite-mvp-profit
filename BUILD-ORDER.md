...
- [~] **SUPERSEDED by item 12.** Original text: `components/walkthroughs/ShopRateWalkthrough.tsx` — 5 per-department rates into `shop_labor_rates`. Pivoted to the first-principles blended-rate flow at [`specs/shop-rate-setup/`](specs/shop-rate-setup/) after the per-dept flow proved wrong in practice: shops think in overhead + labor + billable hours, not per-dept rates. Composer math swaps to `total_hours × orgs.shop_rate` (per-dept hours still captured for scheduling). See item 12.
- [ ] `components/walkthroughs/BaseCabinetWalkthrough.tsx` — embeddable component with **five guided inputs** (Eng / CNC / Machining / Assembly / Finish) for one 8′ run of base cabinets. Machining folds into Assembly on save — the output stored on the org's `rate_book_items` "Base cabinet" row is **four per-LF dept values** (`base_labor_hours_eng/cnc/assembly/finish`). Doors are out of scope here; `DoorStyleWalkthrough` (item 7) owns door calibration against 4 doors × 24″×30″ and scales to Upper/Full via the product multipliers in `lib/products.ts`. The richer 9-step build-order flow at [`specs/base-cabinet-walkthrough/`](specs/base-cabinet-walkthrough/) is deferred as the V2 walkthrough. Same component reused at `/settings/calibration/base`.
...
- [ ] **Item 12 — First-principles shop rate walkthrough (supersedes item 3).** Four screens inside the welcome overlay, replacing the per-department 5-input panel:
  1. **Overhead** — categorized $ inputs (rent, utilities, insurance, software, vehicle, shop consumables, tools, admin, other). Each row toggles between monthly and annual. Rows can be renamed, added, removed. Live annual total.
  2. **Team** — add-row list. Name + annual comp per employee. Hourly employees are converted to annual before persisting (walkthrough does the math).
  3. **Billable hours** — `hrs/wk × weeks/year × utilization%`. Single-crew-equivalent model for V1; multi-crew scaling deferred to the richer post-overlay flow.
  4. **Result** — derived `shop_rate = (annual_overhead + annual_team_comp) / billable_hours`, with override allowed. On re-entry, if the derived value differs from the persisted `orgs.shop_rate`, Result shows both with Update / Keep buttons — user decides each time, no persistent override flag.
  Persistence: three jsonb columns on `orgs` (`overhead_inputs`, `team_members`, `billable_hours_inputs`) per migration 022 — chose jsonb over child tables for V1 settings-form shape, documented in migration header. Composer math swaps to `total_hours × orgs.shop_rate`; `shop_labor_rates` table drops in the same PR (readers routed first). `InstallPrefill` reads `orgs.shop_rate` instead of the per-dept install rate. Per-dept hour capture in base-cabinet and door walkthroughs stays unchanged — the composer just sums the depts before multiplying.
