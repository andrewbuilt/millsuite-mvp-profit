// ============================================================================
// components/portal/PortalProjectView.tsx — the project screen
// ============================================================================
// A PURE view over an already-loaded PortalProject. The route stays thin
// (resolve token → load → notFound); everything visual lives here.
//
// That split is not decoration — it's what makes this screen verifiable. The
// portal can't be exercised locally without a live token against a migrated
// database, so this component is rendered against fixture data to check the
// layout (the same trick the estimate-PDF readability work used). Keep it free
// of data access so that stays possible.
//
                  ...
                  {p.payments.rows.map((r, i) => (
                    <Row key={i}>
                      <span>
                        ...
                        {/* Brass is the "this one wants your attention" colour,
                            so it's earned only by a row with a real due date.
                            "Due on schedule" is not a deadline. */}
                        {r.sublabel ? (
                          <span className="text-[10.5px]" style={{ ...mono, color: r.due ? BRASS : MUTED }}>
                            {r.sublabel}
                          </span>
                        ) : null}
                      </span>
