...
export default function GuidesPage() {
  ...
  const { state, loading, startTour, offerTour, restartTour } = useTours()
  ...
  // Starting the first-job tour from scratch goes through its offer modal —
  // that's where the practice-project choice lives, and skipping straight in
  // would silently decide it for them. Resuming doesn't re-ask.
  function launch(tour: Tour, fromStep: number) {
    if (tour.id === 'first-job' && fromStep === 0) offerTour(tour.id as TourId)
    else startTour(tour.id as TourId, fromStep)
  }
